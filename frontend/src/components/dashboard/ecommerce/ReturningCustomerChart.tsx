...
import { getColor, tooltipFormatter } from '@/lib/chart-utils';
import { months } from '@/lib/theme-data';
...
const getOption = () => ({
  ...
  xAxis: {
    ...
    show: true,
    boundaryGap: false,
    axisLine: { show: true, lineStyle: { color: getColor('300') } },
    axisTick: { show: false },
    axisLabel: {
      showMinLabel: false,
      showMaxLabel: false,
  ...
  yAxis: {
    ...
    splitLine: { show: true, lineStyle: { color: getColor('200') } }
  },
  series: [
    {
      name: 'Fourth time',
      ...
      lineStyle: { type: 'dashed', width: 1, color: getColor('primary-300') },
      itemStyle: { borderColor: getColor('primary-300'), borderWidth: 3 }
    },
    {
      name: 'Third time',
      type: 'line',
...
export default function ReturningCustomerChart() {
  return <EChartBox getOption={getOption} style={{ height: 300 }} />;
}
